  ...
  try {
    ...
    if (type === "video") {
      ...
      const originalExtension = originalFilename
        ...
        .pop()
        ?.toLowerCase();
      extension = ["jpg", "jpeg", "png", "webp", "gif"].includes(
        originalExtension || "",
      )
        ? originalExtension!
        : "jpg";
    ...
    const localUrl =
      type === "cover" ? await saveBookCover(file) : await saveBookVideo(file);
  ...
  try {
    ...
    for (let i = 0; i < booksData.length; i++) {
      ...
      try {
        ...
        console.error(
          `    ❌ Failed to seed: ${book.title} - ${errorMessage}\n`,
        );
      }
    }
